...
void main() {
    ...
    float lambertian = max(dot(lightDir,rotationMatrix * normalColor), 0.1);
    float specular = 0.0;
    ...
    gl_FragColor = vec4(texture2D(diffuseTexture, vertUV).rgb * lambertian, 1.0) + specular * 0.2 * vec4(specColor, 1.0);
